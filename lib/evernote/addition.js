...
function saveNote(values) {
  // Obtain note store
  var noteStore = Utility.getNoteStore(values.authtoken, values.config.evernote.sandbox, values.config.evernote.china);
...
function getVaaniNoteMetaData(values) {
  ...
  var promise = new Promise(function(resolve, reject) {
    Utility.getNoteMeta(values.authtoken, values.config.evernote.sandbox, values.config.evernote.china, function(err, notesMetaData) {
      if (err) {
        reject(err);
      } else {
...
module.exports.addVaaniNoteItem = function(authtoken, itemName, config) {
